    ...
    try {
      ...
      if (res.ok) {
        ...
        const unreadIds = data
          .filter((m) => !m.isRead && (m.senderType === "COMPANY" || m.senderType === "SYSTEM"))
          .map((m) => m.id)
    ...
    <div className="h-[calc(100vh-120px)] flex gap-0 bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      <div className="w-72 border-r border-gray-200 flex flex-col">
        <div className="p-4 border-b border-gray-200">
          <Link
            href="/mypage"
            className="flex items-center gap-1 text-xs text-gray-500 hover:text-[#0D9488] mb-2"
          >
            <ChevronLeft className="size-3.5" />
            マイページに戻る
          </Link>
          <h2 className="font-semibold text-gray-900">メッセージ</h2>
          <p className="text-xs text-gray-500 mt-0.5">{threads.length}件の会話</p>
        </div>
      ...
      <div className="flex-1 flex flex-col">
        {selectedThread ? (
          <>
            {/* ヘッダー */}
            <div className="p-4 border-b border-gray-200">
              <p className="font-medium text-gray-900">{selectedThread.jobTitle}</p>
              <p className="text-xs text-gray-500">企業からのメッセージ</p>
            </div>
